...
export type UpdateEventParameters = {
    title: string;
    description?: string;
    startTime: string;
    endTime: string;
...
export async function updateEvent(eventId: number, parameters: UpdateEventParameters) {
    ...
    if (parameters.eventPicture) {
        formData.append("img", parameters.eventPicture, parameters.eventPicture.name);
